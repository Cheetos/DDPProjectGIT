...
v <- training$hp
w <- testing$hp
xTrain <- data.frame(x1=v,x2=v^2,x3=v^3,x4=v^4,x5=v^5,x6=v^6,x7=v^7)
xTest <- data.frame(x1=w,x2=w^2,x3=w^3,x4=w^4,x5=w^5,x6=w^6,x7=w^7)
...
lm.fit6 <- lm(training$mpg ~ x1 + x2 + x3 + x4 + x5 + x6, data=xTrain)
lm.fit7 <- lm(training$mpg ~ x1 + x2 + x3 + x4 + x5 + x6 + x7, data=xTrain)
...
mseTrain[6] <- mean((predict(lm.fit6,xTrain) - training$mpg)^2)
mseTrain[7] <- mean((predict(lm.fit7,xTrain) - training$mpg)^2)
...
mseTest[6] <- mean((predict(lm.fit6,xTest) - testing$mpg)^2)
mseTest[7] <- mean((predict(lm.fit7,xTest) - testing$mpg)^2)
...
shinyServer (
    function(input,output) {
        ...
        output$msePlot <- renderPlot({
            plot(1:7,mseTrain,type='l',col="blue",ylim=c(0,25), ylab="Mean Squared Error (MST)", xlab="n (Degree of Polynomial)", main="Training Error and Test Error")
            lines(1:7,mseTest,col="red")
        ...
        output$mpgvalue <- renderPrint(
            ...
            if(n() == 1)
                ...
                predict(lm.fit6,data.frame(x1=val(),x2=val()^2,x3=val()^3,x4=val()^4,x5=val()^5,x6=val()^6))
            else
                predict(lm.fit7,data.frame(x1=val(),x2=val()^2,x3=val()^3,x4=val()^4,x5=val()^5,x6=val()^6,x7=val()^7))
        )
    }
)